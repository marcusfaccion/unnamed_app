<div id='user-navigation-plan' class="row">
        ...
        <ul class="list-unstyled">
            ...
            <h3 class='text-white bg-primary text-center border-radius-3'><strong>Alertas <span class="glyphicon glyphicon-bullhorn"></span></strong></h3>
                    ...
                    <?php if(count($alerts[$i])>0):?>
                        ...
                        <li class="top-buffer-4 text-primary"><strong><span class='glyphicon glyphicon-ok'></span> Interdição de via: <?=count($_alerts[$i]['interdicoes'])?></strong></li>    
                        <?php endif;?>

                        <?php if(isset($_alerts[$i]['roubos_e_furtos'])):?>
                        <li class="top-buffer-4 text-primary"><strong><span class='glyphicon glyphicon-ok'></span> Roubos de ciclistas: <?=count($_alerts[$i]['roubos_e_furtos'])?></strong></li>    
                        <?php endif;?>

                        <?php if(isset($_alerts[$i]['perigo_na_via'])):?>
                        <li class="top-buffer-4 text-primary"><strong><span class='glyphicon glyphicon-ok'></span> Perigo na via: <?=count($_alerts[$i]['perigo_na_via'])?></strong></li>    
                        <?php endif;?>

                        <?php if(isset($_alerts[$i]['generico'])):?>
                        <li class="top-buffer-4 text-primary"><strong><span class='glyphicon glyphicon-ok'></span> Genéricos: <?=count($_alerts[$i]['generico'])?></strong></li>    
                        <?php endif;?>
                        <li class="top-buffer-5 text-primary"><strong>Total: <?=count($alerts[$i])?></strong></li>    
    ...
    <?php endforeach;?>
    <div class="col-lg-12 col-xs-12 right-pbuffer-2 left-pbuffer-2">
        <h3 class='text-white bg-default text-center border-radius-3'><strong>Bicicletários <span class="glyphicon glyphicon-home"></span></strong></h3>
        <ul class="list-unstyled left-pbuffer-4 tsize-5">

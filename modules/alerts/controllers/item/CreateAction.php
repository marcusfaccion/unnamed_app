    ...
    public function run()        
    {
        ...
        if($alert->save()){
            ...
            if($this->isAjax){
               ...
               $this->controller->renderAjax('@alerts/views/item/view', ['alert' => $alert, 'alert_type'=>$alert_type, 'alert_user'=>$alert_user, 'isAjax'=>$this->isAjax]);
               \Yii::$app->end(0);
            }
            $this->controller->render('@alerts/views/item/view', ['alert' => $alert, 'alert_type'=>$alert_type, 'alert_user'=>$alert_user,'isAjax'=>$this->isAjax]);
        }
